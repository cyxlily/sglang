...
class EAGLEDraftExtendCudaGraphRunner(DecodeCudaGraphRunner):
    ...
    def __init__(
        ...
        *,
        ...
        self.req_token_width = resolve_num_tokens_per_req(
            phase="draft_extend", server_args=model_runner.server_args
        )
        self.max_bs = max(self.capture_bs)
        self.max_num_token = self.max_bs * self.req_token_width
        ...
        self.extend_seq_lens_cpu = [self.req_token_width] * self.max_bs
        ...
        with torch.device(model_runner.device):
            ...
            extend_seq_lens = torch.full(
                (self.max_bs,), self.req_token_width, dtype=torch.int32
            )
            ...
            next_token_logits_buffer = (
                self.model_runner.graph_shared_output.get_logits_buffer(
                    vocab_size, rows=self.max_bs * self.req_token_width
                )
            )
    ...
    def can_run_graph(self, forward_batch: ForwardBatch):
        ...
        if (
            ...
            and spec_info.num_tokens_per_req != self.req_token_width
        ):
            return False
    ...
    def capture_one_shape(
        ...
        num_tokens = bs * self.req_token_width
        ...
        spec_info = EagleDraftExtendInput(
            ...
            num_tokens_per_req=self.req_token_width,
        )
    ...
    def execute(self, forward_batch: ForwardBatch):
        ...
        if self.require_mlp_tp_gather:
            ...
            max_batch_size = (
                max_num_tokens // self.req_token_width
        ...
        if bs * self.req_token_width != num_tokens:
            ...
            buffers.num_correct_drafts.fill_(self.req_token_width)
            buffers.num_accept_tokens.fill_(self.req_token_width)
            buffers.extend_seq_lens.fill_(self.req_token_width)
        ...
        if forward_batch.extend_seq_lens is not None:
            ...
            buffers.extend_seq_lens[:raw_bs].fill_(self.req_token_width)
        ...
        if self.require_gathered_buffer:
            buffers.global_num_tokens_gpu.fill_(bs * self.req_token_width)
            buffers.global_num_tokens_for_logprob_gpu.fill_(bs * self.req_token_width)
        ...
        if forward_batch.extend_seq_lens_cpu is not None:
            ...
            self.extend_seq_lens_cpu[:raw_bs] = [self.req_token_width] * raw_bs
        if bs > raw_bs:
            self.extend_seq_lens_cpu[raw_bs:bs] = [self.req_token_width] * (bs - raw_bs)
