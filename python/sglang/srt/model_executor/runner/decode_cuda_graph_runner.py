...
class DecodeCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def __init__(
        ...
        *,
        ...
        self.req_token_width = model_runner.decode_num_tokens_per_req(
        ...
        self.capture_bs, self.compile_bs = get_batch_sizes_to_capture(
            model_runner, self.req_token_width
        )
    ...
    def _build_ragged_verify_token_buckets(self) -> list[int]:
        buckets = sorted({bs * self.req_token_width for bs in self.capture_bs})
    ...
    def _ragged_capture_slots(self, num_tokens: int) -> int:
        if envs.SGLANG_TEST_RAGGED_VERIFY_FORCE_UNIFORM_CAPTURE.get():
            return num_tokens // self.req_token_width
    ...
    def _capture_ragged_verify_layout(self, num_tokens: int):
        ...
        verify_lens_cpu = build_capture_verify_lens(
            ...
            num_draft_tokens=self.req_token_width,
        )
    ...
    def can_run_graph(self, forward_batch: ForwardBatch):
        ...
        is_ngram_supported = (
            (
                forward_batch.batch_size * self.req_token_width
    ...
    def capture(self) -> None:
        ...
        if self.enable_torch_compile and not (get_flags().capture.enable_torch_compile):
            ...
            _, self.compile_bs = get_batch_sizes_to_capture(
                self.model_runner, self.req_token_width
            )
    ...
    def capture_one_shape(
        ...
        num_tokens = size * self.req_token_width
    ...
    def load_batch(
        ...
        if not forward_batch.needs_forward_metadata_init():
            ...
            graph_size_key = (
                ...
                if is_ragged
                    ...
                    bs=self.bs, num_tokens=self.bs * self.req_token_width
                )
            )
            if is_ragged:
        ...
        if is_ragged:
            ...
            raw_num_token = raw_bs * self.req_token_width
            if self.require_mlp_tp_gather:
    ...
    def get_spec_info(self, num_tokens: int):
        ...
        if (
            ...
            spec_info = DFlashVerifyInput(
                draft_token=None,
                positions=None,
                draft_token_num=self.req_token_width,
                custom_mask=(
                    None
