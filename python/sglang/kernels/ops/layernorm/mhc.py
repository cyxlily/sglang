...
def _compute_num_split_for_mhc_pre(num_tokens: int, hc_hidden_size: int) -> int:
    ...
    # Use the active accelerator backend when estimating occupancy. On XPU
    # builds, torch.cuda APIs are unavailable and raise AssertionError.
    if torch.xpu.is_available():
        props = torch.xpu.get_device_properties(0)
        n_sms = getattr(props, "multi_processor_count", None)
        if n_sms is None:
            # Intel XPU exposes compute units / subslices rather than CUDA SMs.
            n_sms = getattr(
